...
<script src="http://edgemanagementsystem.co.za/mantis/js/scripts.min.js"></script>
        ...
        if(isset($_POST["Import"])) {
            ...
            if ($_FILES["file"]["size"] > 0) {
                    ...
                    if($count >= 5) {
                        ...
                        if(trim($video_code)!="") {
                            for ($i = 1; $i < $ctr; $i++) {
                                ...
                                if ($ctr - $i > 1) {
                                    if(trim($emapData[$i]) != "") {
                                        $sql = "INSERT into playlist_process(unit_title,unit_id,parent_unit_id,video_code,video_title,vimeo_code) values ('$title','$title_id','$parent_id[$i]','','','')";
                                        $conn->query($sql);
                                        $prev_id = $id;
                                        $id += 1;
                                    }
                                   ...
                                   $title = preg_replace('/&(?!#?[a-z0-9]+;)/', '&amp;', $title);
                                    $video_title = preg_replace('/&(?!#?[a-z0-9]+;)/', '&amp;', $video_title);
                                    $sql = "INSERT into playlist_process(unit_title,unit_id,parent_unit_id,video_code,video_title,vimeo_code) values ('$title','$title_id','$parent_id[$i]','$video_code','$video_title','$vimeo_code')";
                                    $conn->query($sql);
                                    $prev_id = $id;
                                    $id += 1;
                ...
                echo "<h3>
                    <div class='row'>
                    <div class='col_1-2'><a href='output_playlist_new.php?xml_type=playlist&xml_doc_tagline=learning area 2&xml_doc_title=accounting 2&colors[]=$colors[0]&colors[]=$colors[1]&colors[]=$colors[2]&colors[]=$colors[3]&colors[]=$colors[4]&colors[]=$colors[5]&colors[]=$colors[6]&colors[]=$colors[7]&content=$content&xmlID=$xmlID&client=$client&clienturl=$clienturl&aid=$aid&sku=$sku'>Create Playlist XML</a></div>
                    </div>
                    <div class='row'>
                    <div class='col_1-2'><a href='output_playlist_new.php?xml_type=DVD&xml_doc_tagline=learning area 2&xml_doc_title=accounting 2&colors[]=$colors[0]&colors[]=$colors[1]&colors[]=$colors[2]&colors[]=$colors[3]&colors[]=$colors[4]&colors[]=$colors[5]&colors[]=$colors[6]&colors[]=$colors[7]&content=$content&client=$client&clienturl=$clienturl&aid=$aid&sku=$sku'>Create DVD XML</a></div>
                    </div></h3>";
            } else {
